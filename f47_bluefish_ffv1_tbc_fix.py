    ...
    if len(sys.argv) < 2:
        ...
        if os.path.exists(fullpath):
            ...
            if 'MATCH' in result:
                ...
                if "PASS!" in result:
                    logger_list.append(f"{outpath} passed the policy checker and it's Matroska can be deleted")
                    try:
                        # Delete FFV1 mkv after successful transcode to V210 mov
                        logger_list.append("*** DELETION OF MKV FOLLOWING SUCCESSFUL TRANSCODE: %s", fullpath)
                        os.remove(fullpath)
                    except Exception:
