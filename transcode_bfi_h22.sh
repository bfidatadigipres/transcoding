...
function log {
    ...
    echo "$1"
    echo -e "$timestamp\t$INPUT\t$1" >> "${LOG_PATH}h22_bfi_transcode.log"
}

# Function to check for control json restrictions
function control {
...
if [ "$EVENT" = "created" ]; then
    ...
    # Only process FFprobe confirmed content
    ...
    if diff "$FRAMEMD5_MOV" "$FRAMEMD5_MKV" > /dev/null; then
        ...
        echo "mv ${TMP} ${DST}" >> "${LOG_PATH}h22_bfi_transcode_success.txt"
...
parallel < "{LOG_PATH}h22_bfi_transcode_success.txt"
